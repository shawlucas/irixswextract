...
static const struct Archive *load_archive(const char *name) {
  ...
  loadedArchivesCount++;
  loadedArchives =
      realloc(loadedArchives, loadedArchivesCount * sizeof(*loadedArchives));
    ...
    if (*p == '/') {
      *p = 0;
      if (mkdir(path, S_IRWXU) != 0 && errno != EEXIST)
        fatal_error("failed to create directory '%s': %s", path,
                    strerror(errno));
      *p = '/';
    }
  }
...
static void write_output_file(const char *filename, const void *data,
                              size_t size, int perms, int decompress) {
...
static const void *get_file_data(const struct Archive *archive,
                                 char *filename) {
